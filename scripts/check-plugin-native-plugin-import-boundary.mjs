...
const baselinePath = path.join(
  ...
  "test",
  "fixtures",
  "plugin-native-plugin-import-boundary-inventory.json",
);
...
function classifyResolvedNativePluginReason(kind, resolvedPath) {
  const verb =
    kind === "export"
      ? "re-exports"
      : kind === "dynamic-import"
        ? "dynamically imports"
        : "imports";
  if (/^native-plugins\/[^/]+\/src\//.test(resolvedPath)) {
    return `${verb} native plugin implementation from src/plugins`;
  }
  if (/^native-plugins\/[^/]+\/index\.[^/]+$/.test(resolvedPath)) {
    return `${verb} native plugin entrypoint from src/plugins`;
  }
  return `${verb} native-plugin-owned file from src/plugins`;
}
    ...
    if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier)) {
      ...
      if (resolvedPath?.startsWith("native-plugins/")) {
        pushEntry(entries, {
          ...
          specifier,
          resolvedPath,
          reason: classifyResolvedNativePluginReason("import", resolvedPath),
        });
      }
    } else if (
      ...
      if (resolvedPath?.startsWith("native-plugins/")) {
        pushEntry(entries, {
          ...
          specifier,
          resolvedPath,
          reason: classifyResolvedNativePluginReason("export", resolvedPath),
        });
      }
    } else if (
      ...
      if (resolvedPath?.startsWith("native-plugins/")) {
        pushEntry(entries, {
          ...
          specifier,
          resolvedPath,
          reason: classifyResolvedNativePluginReason("dynamic-import", resolvedPath),
        });
      }
    }
...
export async function collectPluginNativePluginImportBoundaryInventory() {
...
export const collectPluginExtensionImportBoundaryInventory =
  collectPluginNativePluginImportBoundaryInventory;
...
function formatInventoryHuman(inventory) {
  if (inventory.length === 0) {
    return "Rule: src/plugins/** must not import native-plugins/**\nNo plugin import boundary violations found.";
  }

  const lines = [
    "Rule: src/plugins/** must not import native-plugins/**",
    "Plugin native-plugin import boundary inventory:",
  ];
  let activeFile = "";
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const actual = await collectPluginNativePluginImportBoundaryInventory();

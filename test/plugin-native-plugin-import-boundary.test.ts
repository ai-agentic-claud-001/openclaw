...
import {
  ...
  diffInventory,
} from "../scripts/check-plugin-native-plugin-import-boundary.mjs";

const repoRoot = process.cwd();
const scriptPath = path.join(repoRoot, "scripts", "check-plugin-native-plugin-import-boundary.mjs");
...
describe("plugin native-plugin import boundary inventory", () => {
  it("keeps web-search-providers out of the remaining inventory", async () => {
    const inventory = await collectPluginNativePluginImportBoundaryInventory();
  ...
  it("produces stable sorted output", async () => {
    const first = await collectPluginNativePluginImportBoundaryInventory();

...
const baselinePathByMode = {
  "src-outside-plugin-sdk": path.join(
    ...
    "test",
    "fixtures",
    "native-plugin-src-outside-plugin-sdk-inventory.json",
  ),
  "plugin-sdk-internal": path.join(
    repoRoot,
    "test",
    "fixtures",
    "native-plugin-plugin-sdk-internal-inventory.json",
  ),
  "relative-outside-package": path.join(
    repoRoot,
    "test",
    "fixtures",
    "native-plugin-relative-outside-package-inventory.json",
  ),
};

const ruleTextByMode = {
  "src-outside-plugin-sdk":
    "Rule: production native-plugins/** must not import src/** outside src/plugin-sdk/**",
  "plugin-sdk-internal":
    "Rule: production native-plugins/** must not import src/plugin-sdk-internal/**",
  "relative-outside-package":
    "Rule: production native-plugins/** must not use relative imports that escape their own native plugin package root",
};
...
async function collectNativePluginSourceFiles(rootDir) {
  const out = [];
...
function resolveNativePluginRoot(filePath) {
  const relativePath = normalizePath(filePath);
  const segments = relativePath.split("/");
  if (segments[0] !== "native-plugins" || !segments[1]) {
    return null;
  }
...
function classifyReason(mode, kind, resolvedPath, specifier) {
  ...
  if (mode === "relative-outside-package") {
    ...
    if (resolvedPath?.startsWith("src/")) {
      return `${verb} core src path via relative path outside the native plugin package`;
    }
    if (resolvedPath?.startsWith("native-plugins/")) {
      return `${verb} another native plugin via relative path outside the native plugin package`;
    }
    return `${verb} relative path ${specifier} outside the native plugin package`;
  }
  if (mode === "plugin-sdk-internal") {
    return `${verb} src/plugin-sdk-internal from a native plugin`;
  }
  if (resolvedPath.startsWith("src/plugin-sdk/")) {
    return `${verb} allowed plugin-sdk path`;
  }
  return `${verb} core src path outside plugin-sdk from a native plugin`;
}
...
function collectFromSourceFile(mode, sourceFile, filePath) {
  const entries = [];
  const nativePluginRoot = resolveNativePluginRoot(filePath);

  function push(kind, specifierNode, specifier) {
    const resolvedPath = resolveSpecifier(specifier, filePath);
    if (mode === "relative-outside-package") {
      if (!specifier.startsWith(".") || !resolvedPath || !nativePluginRoot) {
        return;
      }
      if (resolvedPath === nativePluginRoot || resolvedPath.startsWith(`${nativePluginRoot}/`)) {
        return;
      }
...
export async function collectNativePluginPluginSdkBoundaryInventory(mode) {
  if (!MODES.has(mode)) {
    throw new Error(`Unknown mode: ${mode}`);
  }
  const files = await collectNativePluginSourceFiles(nativePluginsRoot);
  const inventory = [];
...
export const collectExtensionPluginSdkBoundaryInventory =
  collectNativePluginPluginSdkBoundaryInventory;
...
function formatInventoryHuman(mode, inventory) {
  ...
  if (inventory.length === 0) {
    lines.push("No native plugin plugin-sdk boundary violations found.");
    return lines.join("\n");
  }
  lines.push("Native plugin boundary inventory:");
  let activeFile = "";
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const actual = await collectNativePluginPluginSdkBoundaryInventory(mode);
  if (json) {

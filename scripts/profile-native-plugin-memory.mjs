...
function printHelp() {
  console.log(`Usage: node scripts/profile-native-plugin-memory.mjs [options]

Profiles peak RSS for built native plugin entrypoints in dist/native-plugins/*/index.js.
Run pnpm build first if you want stats for the latest source changes.

Options:
  --native-plugin, -e <id>     Limit profiling to one or more native plugin ids (repeatable)
  --concurrency <n>        Number of per-native-plugin workers (default: ${DEFAULT_CONCURRENCY})
...
Examples:
  pnpm test:native-plugins:memory
  pnpm test:native-plugins:memory -- --native-plugin discord
  pnpm test:native-plugins:memory -- --native-plugin discord --native-plugin telegram --skip-combined
`);
}
...
function findExtensionEntries(repoRoot) {
  const extensionsDir = path.join(repoRoot, "dist", "native-plugins");
  if (!existsSync(extensionsDir)) {
    throw new Error("dist/native-plugins not found. Run pnpm build first.");
  }
  ...
  if (entries.length === 0) {
    throw new Error(
      "No built native plugin entrypoints found under dist/native-plugins/*/index.js",
    );
  }
  return entries;
}
...
async function main() {
  ...
  if (missing.length > 0) {
    throw new Error(`Unknown built native plugin ids: ${missing.join(", ")}`);
  }
  if (selectedEntries.length === 0) {
    throw new Error("No native plugins selected for profiling");
  }

  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "openclaw-native-plugin-memory-"));
  const hookPath = path.join(tmpHome, "measure-rss.mjs");
  const jsonPath = options.jsonPath ?? path.join(os.tmpdir(), "openclaw-native-plugin-memory.json");
  ...
  try {
    ...
    async function worker() {
      while (pending.length > 0) {
        ...
        console.log(`[native-plugin-memory] ${next.dir}: ${status} ${rss}`);
      }
    }
    ...
    console.log(`[native-plugin-memory] report: ${jsonPath}`);
    console.log(
      JSON.stringify(
        {
...
try {
  ...
  console.error(`[native-plugin-memory] ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
}
